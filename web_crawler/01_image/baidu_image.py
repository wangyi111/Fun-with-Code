...
def get_img(page, headers,dir_name):
    reg = re.compile('https://.*?\.jpg')
    imglist = re.findall(reg, page)[::3]
    x = 0
    for imgurl in imglist:
        with open(dir_name + '/' + '%s.jpg' % x+1, 'wb') as file:
            file.write(requests.get(imgurl, headers=headers).content)
            x += 1
        print('downloading:',x,'/',len(imglist),end = '\r')
    print()
    print('downloaded %d images' %len(imglist))

if __name__ == '__main__':
    page = get_page(url, date, headers)
    dir_name = 'baidu_wmImage'
    if not os.path.exists(dir_name):
        os.mkdir(dir_name)
    get_img(page, headers,dir_name)
